...
$view = new view(array('apiFolder' => $apiFolder, 'apiVersion' => $webAPI, 'webVersion' => $webVersion, 's'=>$s, 'limit'=>$limit, 'wrap'=>$wrap, 'shuffle'=>$shuffle, 'showID'=>$showID, 'showUser' => $showUser, 'js' => $js));
...
class view {
    ...
    // List of args the view class can accept
    public $allowedArgs = ['apiFolder', 'apiVersion', 'webVersion', 's', 'limit', 'wrap', 'shuffle', 'showID', 'showUser', 'js'];

    function __construct($args=[]) {

        // Loop through list of allowedArgs and see if they're set. If they're not, make them an empty string
        foreach($this->allowedArgs as $key) {
            $this->$key = $args[$key] ?? '';
        }
    }
    ...
    public function search($args=[]) {
        ...
        $shuffleChecked = ($this->shuffle == 1) ? "checked" : null;
        $showUserChecked = ($this->showUser == 1) ? "checked" : null;
        $showIDChecked = ($this->showID == 1) ? "checked" : null;
        $submitVisible = "<input id='searchSubmit' type='submit' value='Search' />"; // Submit button only needs to be shown if javascript is disabled
        $searchVisible = ($_SESSION['web']['searchVisible'] == 1) ? null:"fade";
        return "
        <div id='search' class='fadeBox {$searchVisible}'>
        <form id='searchForm' method='get' action='index.php'>
        <input type='hidden' name='f' value='search'>
            <p><input type='text' id='searchbox' name='s' placeholder='Search...' value='{$this->s}' /><p>
            <p>Tag: <select id='searchTag' name='tag'>
            <option value='all'>All</option>
            ".$this->tagOptions()."
            </select></p>
            <p><label>Limit: <input type='number' name='limit' value='{$this->limit}' size='4' /></label> <label>wrap: <input type='text' name='wrap' value='{$this->wrap}' size='3'></label></p>
    ...
    function footer() {
        $ret = array(null,null,null,null,null); // Return variables
    
        if ($this->js != 1)
            $ret[0] = "<a href='?js=1' data-toggle='' class='' title='Enable JavaScript'>HTML Mode</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;";
        else 
            $ret[0] = "<noscript>HTML Mode&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;</noscript>";
        

        if ($_SESSION['web']['create'] == 1)
            $ret[1] = "<a href='#' data-toggle='create' class='divToggle'>Create</a>&nbsp;&nbsp;&nbsp;";

        if ($_SESSION['web']['search'] == 1)
            $ret[2] = "<a href='#' data-toggle='search' class='divToggle'>Search</a>&nbsp;&nbsp;&nbsp;";

        if ($_SESSION['web']['info'] == 1)
            $ret[3] = "<a href='#' data-toggle='info' class='divToggle'>API</a>&nbsp;&nbsp;&nbsp;";

        if ($_SESSION['web']['github'] == 1)
            $ret[4] = "<a href='https://github.com/cata-lystic/redbot-cogs/tree/main/thoughts' target='_blank'>GitHub</a>&nbsp;&nbsp;&nbsp;";

        if ($_SESSION['web']['versionVisible'] == 1)
            $ret[5] = "<span title='API Version: ".$this->apiVersion."'>Thoughts ".$this->webVersion."</span>";
